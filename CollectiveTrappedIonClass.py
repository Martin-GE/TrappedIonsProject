...
class CollectiveTrappedIon(CouplingModel, MPOModel):
    r"""
    ...
    def __init__(self, model_params):
        ...
        for i in range(0, 2*L, 2):
            for n in range(1, 2*L, 2):
                ...
                if i < n:
                    self.add_coupling_term(- g_in, i, n, 'IdPlusSigmaz', 'BdPlusB')
                elif n < i:
                    self.add_coupling_term(- g_in, n, i, 'BdPlusB', 'IdPlusSigmaz')
        ...
        if Trap == 'Paul Traps':
            for i in range(0, 2*L, 2):
                for j in range(0, 2*L, 2):
                    # we have to differenciate the two following cases
